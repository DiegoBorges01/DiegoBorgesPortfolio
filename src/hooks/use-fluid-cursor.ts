...
// WebGL fluid cursor simulation. Adapted from PavelDoGreat/WebGL-Fluid-Simulation.
// Mounts on a #fluid canvas. Desktop-only — caller decides when to invoke.

const initFluidCursor = (canvasEl?: HTMLCanvasElement | null) => {
  const canvas = canvasEl ?? (document.getElementById("fluid") as HTMLCanvasElement | null);
  if (!canvas) return;
  let stopped = false;
  let rafId = 0;
  resizeCanvas();
